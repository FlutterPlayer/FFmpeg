...
int av_dict_parse_string(AVDictionary **pm, const char *str,
  ...
  while (*str) {
    if ((ret = parse_key_value_pair(pm, &str, key_val_sep, pairs_sep, flags)) <
        0)
      return ret;
...
int av_dict_get_string(const AVDictionary *m, char **buffer,
  ...
  while ((t = av_dict_get(m, "", t, AV_DICT_IGNORE_SUFFIX))) {
    ...
    av_bprint_escape(&bprint, t->key, special_chars, AV_ESCAPE_MODE_BACKSLASH,
                     0);
    av_bprint_append_data(&bprint, &key_val_sep, 1);
    av_bprint_escape(&bprint, t->value, special_chars, AV_ESCAPE_MODE_BACKSLASH,
                     0);
  }
